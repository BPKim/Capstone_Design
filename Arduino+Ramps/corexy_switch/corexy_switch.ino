...
boolean de_position = true; // true : right up / false : left down
...
void loop () {
  ...
  if((!default_start)&&(!default_endend)){
    ...
    while(((mornal=digitalRead(NORMAL)) == HIGH)&&((fast=digitalRead(FAST)) == HIGH)){}
    
    default_start = true;
    delay(1);
    
    if(mornal != _mornal){
        Serial.print("Mornal Mode!!!\n");
        _mornal = mornal;
        mode = true;
     }else if(fast != _fast){
     ...
     // 기본 위치로
  while(default_start && de_position){
  ...
  y_max = digitalRead(Y_MAX_PIN);
    ...
    if(!de_position){
      if(y_min != y_tin)
      {
        ...
        while((goLeft<14300)&&((x_max = digitalRead(X_MAX_PIN))==HIGH)){
           left();
           goLeft++;
        }
        goLeft=0;
    ...
    }else  // inverse
    {
       ...
       if(y_min != y_tin)
      {
        ...
        while((goLeft<14300)&&((x_min = digitalRead(X_MIN_PIN))==HIGH)){
            right();
           goLeft++;
        }
        goLeft=0;
